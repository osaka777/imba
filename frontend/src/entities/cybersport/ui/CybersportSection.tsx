...
type CybersportSectionProps = {
  className?: string;
};

/**
 * /cybersport — same live chrome as /live (sidebar + tournament rows),
 * but cybersport-only menu and feed.
 */
export function CybersportSection({ className }: CybersportSectionProps) {
  const { t } = useLocale();
  const { sport, setSport, syncUrlSport, hydrated } = useCyberSportPreference("");
  ...
  return (
    <div className={cn(gamesStyles.Games, className)}>
          ...
          <div className={shellStyles.pageMain}>
            ...
            <div className={shellStyles.pageMainBody}>
              ...
              >
                ...
                {showLoader ? (
                  <LoadingSpinner className={gamesStyles.loading} key="cyber-live-loading" />
                ) : null}
              </InfiniteScroll>
            </div>
          </div>

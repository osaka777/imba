...
type DcLabel = "1X" | "12" | "X2";

type WcMatchDcCellProps = {
  event: WcEvent;
  label: DcLabel;
  value: string;
};

export function WcMatchDcCell({ event, label, value }: WcMatchDcCellProps) {
  const bettingOpen = useWcBettingOpen(event);
  const outcomeKey =
    label === "1X" ? "DC_1X" : label === "12" ? "DC_12" : "DC_X2";
  const market = wcMarketId("double_chance", outcomeKey);
  ...
  const toggleRate = useCallback(() => {
    ...
    setRates((prev) => {
      ...
      return [
        ...
        buildWcDcRate(event, label, odd),
      ];
    });
    window.dispatchEvent(new CustomEvent("open-coupon"));
  }, [event, isAvailable, label, market, setRates, value]);
  ...
  }, [value, event, label, market, isRateAdded, setRates, bettingOpen]);

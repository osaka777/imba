...
export const CouponWrapper: React.FC<CouponWrapper> = ({ className }) => {
  ...
  const couponPanel = (
    <div
      className={`${styles.CouponWrapper} ${isOpen && styles.CouponWrapper_open} ${isDesktop ? className : ""}`}
      onClick={(e) => {
  ...
  return (
    <>
      {portalReady && !isDesktop ? createPortal(couponPanel, document.body) : couponPanel}
      <Button
        className={`${styles.trigger} ${isOpen ? styles.trigger_hidden : ""}`}
